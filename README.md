...
- A-M1 第 1 步（2026-08-14）：**Android 工程落地**——Gradle 双模块骨架（core 纯算法 / app 壳）、`Geo.kt`/`RecorderState.kt`/`JsonStore.kt`/`GpsErrors.kt` 从 TS 直译、**38 项对照测试**（对应网页版 geo/state/db/gps 测试 + 并发冒烟）；Android CI 云端构建通过（测试全绿 + debug APK 产物）；修复原子写截断 bug（FileOutputStream 默认打开截断文件 → append 模式 fsync）。
