...
- 📱 **Android 原生版迁移（代码完成，待真机验收）**：[迁移 SPEC](./android/SPEC.md) · [数据格式契约](./android/数据格式.md) · [防杀与后台定位](./android/防杀与后台定位.md) · [开发环境与流程](./android/开发环境与流程.md) · [真机验收清单](./android/真机验收清单.md) · [高德接入指引](./android/高德接入指引.md)
...
- A-M5（2026-08-14）：**A-M5 代码完成，Android 迁移全部里程碑收官**——应用图标（灯笼五档 mipmap）、**JSON 导出/导入**（FileProvider 分享 + 系统文件选择器，与网页版互通闭环）、`importAllJson` 契约测试、三份交付文档（A-M5 规格/真机验收清单/高德接入指引）；Android CI 全绿（core 107 + app 8 = **115 项**）。
- A-M4 第 2 步（2026-08-14）：**A-M4 代码完成**——清单管理页、回顾页漏访标红/套名芯片、记录页清单入口；Android CI 全绿（114 项）。
- A-M4 第 1 步（2026-08-14）：**core 直译完成**——PlanOps（清单/漏访匹配/套名候选）+ clearPlan + 10 项对照测试。
- A-M3 第 2 步（2026-08-14）：**A-M3 代码完成**——三线对比页（成绩单四卡 + 推演动画 + 压轴 morph）+ 回顾页入口 + 历史列表绕路率；Android CI 全绿（104 项）。
- A-M3 第 1 步（2026-08-14）：**core 直译完成**——Tsp/Optimize/Polyline + 19 项对照测试；昌乐县 15/20 户仿真数值与网页版逐位一致；修复幽灵边 bug。
- A-M2 第 2 步（2026-08-14）：**A-M2 代码完成**——历史列表页、回顾页（Canvas 回放动画 + 收拾工具）、页面导航；Android CI 全绿（85 项）。
...
- 前置任务·补充（2026-08-13）：新增 `android/开发环境与流程.md`；SPEC §9 双版本共存策略定案。
