...
| 测试 | vitest（145 项单测）+ jsdom（UI 冒烟）+ fake-indexeddb |
...
### 逻辑链整理与漂移根治（2026-08-20 ~ 21）

| 日期 | 事项 | 内容 |
|---|---|---|
| 08-21 | **v1.0.10 朝向「探照灯」（版本码 7）** | 仿高德/百度：手机朝哪儿，地图当前位置点就向哪个方向打出一束高德式蓝色光锥（60° 扇形、边缘径向淡出），随手机旋转实时平滑转动。实现：安卓 `ROTATION_VECTOR` 传感器（系统融合磁力计+加速度计）→ 纯四元数数学取「用户视线（设备 -Z 轴）的水平投影方位」→ `GeomagneticField` 按当前位置磁偏角校正成真北 → 经 JS 桥 `BaibaiMap.setHeading` 只改光锥 `transform`（不重建 DOM、不重绘地图）→ JS 侧低通滤波（α=0.35）+ 最短弧插值（359°→1° 顺转 2° 不逆转）。关键取舍：不用官方 compass 的 `remapCoordinateSystem(AXIS_X,AXIS_Z)+getOrientation` 写法（其给出屏幕法线方位，竖持面北会差半圈）；节流 15Hz + 变化 ≥2° 才推；进记录页注册传感器/离开注销（省电）；无传感器设备（老机型/模拟器）光锥静默隐藏。方案设计见 [朝向探照灯方案.md](./docs/朝向探照灯方案.md) |
| 08-21 | **v1.0.9 记录用时走字修复（版本码 6）** | 真机主诉「点开始拜年后顶部记录用时恒 0，历史页却有用时」的根因：安卓 `TickClock` 的 `tick` 状态只在协程里自增、组合期间从未被读取——Compose 只订阅「组合期间读取的 State」，无人读取的写入不触发重组，`Text` 恒停首帧 00:00:00（v1.0.7 修的是 `elapsedMs()` 计算层，显示层重组失效未被发现，单测测计算测不到 UI 重组）。修复：每秒把 `elapsedMs()` 拉进 State、`Text` 参数直接读它，订阅链恢复每秒刷新（等定位阶段同样走字） |
| 08-21 | **v1.0.8 按钮逻辑链整理（版本码 5）** | ① 删除无实际用途的「撤销上点」按钮（两端，核心撤销能力保留在数据层）；② 「结束复盘」清理与单击并存的长按死代码，统一为单击触发（距 Home >20m 时弹窗二次确认防误触，与安卓端一致）；③ 回顾页「返回」来源感知（两端）：从历史页进入 → 返回历史页；结束复盘自动进入 → 返回记录页可立即开始下一场（此前一律跳历史页，逻辑链断裂）；④ 仓库瘦身：移除设计期遗留（uxpilot-export/ 六张设计稿导出、layout-preview.html 重构方案稿、线上首页「查看手机端重构方案」浮动入口、metadata.json），README 测试数 111→145 |
| 08-21 | **v1.0.7 R9 概率性漂移根治 + 三项体验修复（版本码 4）** | ① 静坐漂移「一阵一阵」根治：三重过滤——平滑窗未满 3 个采样不入库、静止门槛随时长抬升（0.5m/s，封顶 50m）、连续 2 次确认才入库；R9.1 修正静止时长从段起点起算（在户内停留的几分钟不再抬高门槛导致短段零入库丢边）；② 用时恒 0 修复：计时从点击「开始拜年」那刻起算（含等定位时间），不用首点时刻（首点迟到曾致计时回跳）；③ 定位只取一次：10 分钟内已有 GPS 定位直接复用定 Home，切页/重开不再重新等待（缓存只认 GPS 点，拒绝网络点混入坐标系冲突）；④ App 显示名改「拜拜」；距离显示精度 0.1km→0.01km。双端全量：Web 145 项 / Android core 131 项 |
| 08-20 | **v1.0.6 R8 静止漂移 + M2 新版首页（版本码 3）** | R8：中位数平滑窗口（5 点滑窗）+ 精度自适应门槛（min(max(5m, 精度), 30m)）+ 入库后窗口重置；M2：首页对齐网页版智驾设计稿——满屏地图 + 玻璃看板 + 底部座舱，保留全部原功能（Web/Android 双端） |

### 真机验收准备（2026-08-20）

| 日期 | 事项 | 内容 |
|---|---|---|
| 08-20 | **R7 双修：静止轨迹复杂 + 结束后无「开始拜年」按钮（v1.0.5）** | 真机两大主诉：① 坐着不动 1~2 分钟，结束拜年后仍画出多条线段的复杂轨迹——addPoint 无条件全收 GPS 抖动点（3~10m）。修复：addPoint 两道入口闸门——跳变点（2s 内 >100m，人力不可达）直接丢弃不入库 + 静止最小位移过滤（距上一入库点 <5m 不入点）；② 拜完一次年回记录页主按钮永远消失——安卓 `_session` 残留 FINISHED 快照。修复：跳转回顾页后 `consumeFinishedSession()` 清快照对齐网页版 `complete()` 语义（一天可拜多次年）。另修 `projectToView` 最小视野跨度 60m（静止小点团不再放大充满视口，仅小跨度场景生效、大跨度投影逐位不变）。两端同步 + 测试：网页 137 项（新增静止过滤 2 项/投影 1 项/跳变不入库断言）、安卓 core/app 测试同步（RecorderHubTest 新增结束后再开始回归） |
