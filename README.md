...
- 修复（2026-08-13）：**致命修复——所有按钮失灵**（根因：四视图 `$` 选择器缺 `#` 前缀导致事件绑定崩溃，UI 渲染但无响应）；emoji 图标全面替换为 SVG 线性图标（走路/骑车/清单/历史/三线对比/推演/压轴）；背景装饰重绘（可辨认的烟花放射线与鞭炮串）；交互逻辑补全（出行方式按钮按状态禁用 + 切换反馈提示）；新增 **jsdom UI 冒烟测试**（防回归，测试增至 107 项）；Service Worker 缓存版本 v2。
